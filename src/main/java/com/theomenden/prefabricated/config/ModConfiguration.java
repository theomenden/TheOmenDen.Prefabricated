...
public class ModConfiguration implements ConfigData {
    ...
    protected void initialize() {
        Arrays
                .stream(ConfigKeyNames.Keys)
                .filter(key -> !this.recipes.containsKey(key))
                .forEach(key -> this.recipes.put(key, true));
        ...
        for (EnumBasicStructureName value : EnumBasicStructureName.values()) {
            ...
            if (options.size() > 1) {
                ...
                options
                        .stream()
                        .filter(option -> !structureOptions.containsKey(option.getTranslationString()))
                        .forEach(option -> structureOptions.put(option.getTranslationString(), true));
        ...
        String structureOptionsKey = "item.prefabricated.item_house";
        ...
        Arrays
                .stream(HouseConfiguration.HouseStyle.values())
                .filter(houseStyle -> !houseOptions.containsKey(houseStyle.getTranslationString()))
                .forEach(houseStyle -> houseOptions.put(houseStyle.getTranslationString(), true));

        // Add the Improved house settings.
        structureOptionsKey = "item.prefabricated.item_house_improved";
        ...
        Arrays
                .stream(HouseImprovedConfiguration.HouseStyle.values())
                .filter(houseStyle -> !houseImprovedOptions.containsKey(houseStyle.getTranslationString()))
                .forEach(houseStyle -> houseImprovedOptions.put(houseStyle.getTranslationString(), true));

        // Add the Advanced house settings.
        structureOptionsKey = "item.prefabricated.item_house_advanced";
        ...
        Arrays
                .stream(HouseAdvancedConfiguration.HouseStyle.values())
                .filter(houseStyle -> !houseAdvancedOptions.containsKey(houseStyle.getTranslationString()))
                .forEach(houseStyle -> houseAdvancedOptions.put(houseStyle.getTranslationString(), true));
    }
    ...
    public CompoundTag writeCompoundTag() {
        ...
        this.structureOptions
                .forEach((key, value) -> {
                    ...
                    value
                            .forEach(mainItem::putBoolean);
                    structureOptionTag.put(key, mainItem);
                });
    ...
    public void readFromTag(CompoundTag tag) {
        ...
        Arrays
                .stream(ConfigKeyNames.Keys)
                .forEach(key -> this.recipes.put(key, tag.getBoolean(key)));
